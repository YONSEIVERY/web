...
> **2026-08-08 추가**: 이 문서는 5/19 동결 스냅샷이다. 이후 구축된 리크루팅
> 시스템, 학회원 포털, 백업 체계는 `ops-handbook.md`와 git log가 기준이다.
> 6절 미해결 사항 중 사이트 메타 자산(OG·robots·sitemap), 커스텀 `/not-found`,
> 본문 중앙 정렬은 해소됐다. 8절 배포 흐름도 낡았다: develop 브랜치는 폐지,
> main 단일 브랜치(push = production)로 운영한다.
