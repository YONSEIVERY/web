...
- **어드민 등급은 두 단계다 (0032).** `admins.tier`가 `lead`(학회장단) 또는
  `officer`(임원진). 신규 등록은 default로 officer가 되고, lead 승격은 SQL로만
  한다. officer가 막히는 것: 합불 변경, 결과 메일 발송, 지원자·참관자 명단
  반출(엑셀·CSV·전화번호 일괄 복사), 접수 열기·마감 변경, 자율 등록 승인·거절,
  합격자 일괄 등록, 모든 영구 삭제. 열람은 전부 열려 있다.
...
## 5. 마이그레이션 이력 (운영 DB 적용 완료: 0001~0035. 0036은 적용 대기, 다음 번호는 0037부터)
...
**0035 세션 카테고리(kind CHECK를 regular·insight·study·convention·special로
확장하고, 43기와 44기의 special 회차를 제목으로 골라 각 종류로 이관한다.
아이디어톤과 외부 교육은 special로 남는다. 인사이트가 주차별로 쌓이면서
special 한 통에 세 종류가 뒤섞이는 것을 막고 비정규 세션을 사이드바 메뉴로
가른다)** / **0036 학회원 기록 첨부 허용 여부(club_sessions.post_attachments
boolean, 기본 true. 인사이트 소감문처럼 글만 받는 회차에서 사진·파일 칸을
숨기고 서버 액션도 거부한다. 기존 insight 회차는 글 전용으로 백필)**.
새 마이그레이션은 파일 추가 후 Supabase SQL Editor에서 수동 실행한다.
**어느 세션이 만들든 다음 번호는 0037부터다** (Operator 인계분 포함. 0029~0036은
2026-09-02~08에 Builder·Supervisor가 사용했다).

**0033·0034·0035·0036 적용 순서 주의**: SQL을 먼저 돌리고 코드를 머지한다. 0033과
0034, 0036은 컬럼이 없는 DB에 새 코드가 나가면 세션 저장이 실패한다(PostgREST가
미지의 컬럼을 거절한다). 0035는 CHECK 제약이 걸려 있어, 임원진이 새 유형을 고르는
순간 저장이 거절된다. 어느 쪽이든 반대 순서로 두면 세션 편집 화면이 멈춘다.

**마이그레이션 SQL은 한 문장을 한 줄로 쓴다.** 0034에서 실행 도구가 문장을
중간에서 잘라 두 번 연속 실패했다. do 블록을 쓰지 않고(조건부 제약은 drop
constraint if exists 뒤에 add constraint), alter table의 add column을 콤마로
이어 여러 줄에 걸치지 않는다. SQL Editor는 텍스트가 선택돼 있으면 선택 영역만
실행한다. 실패하면 먼저 조회로 부분 적용 여부를 확인한다.
