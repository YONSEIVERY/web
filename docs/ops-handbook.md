...
## 5. 마이그레이션 이력 (운영 DB 적용 완료: 0001~0034. 0035는 적용 대기, 다음 번호는 0036부터)
...
special 한 통에 세 종류가 뒤섞이는 것을 막고 포털 홈을 탭으로 가른다)**.
새 마이그레이션은 파일 추가 후 Supabase SQL Editor에서 수동 실행한다.
**어느 세션이 만들든 다음 번호는 0036부터다** (Operator 인계분 포함. 0029~0035는
2026-09-02~08에 Builder·Supervisor가 사용했다).

**0033·0034·0035 적용 순서 주의**: SQL을 먼저 돌리고 코드를 머지한다. 0033과
0034는 컬럼이 없는 DB에 새 코드가 나가면 세션 저장이 실패한다(PostgREST가 미지의
컬럼을 거절한다). 0035는 CHECK 제약이 걸려 있어, 임원진이 새 유형을 고르는 순간
저장이 거절된다. 어느 쪽이든 반대 순서로 두면 세션 편집 화면이 멈춘다.
